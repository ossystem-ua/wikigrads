...
(function($){
    ...
    var slideItems=carousel.children('.slideItem'),
        ...
        var setSlideSize=function(p){var o=settings,n=numberSlides,w=o.frontWidth,h=o.frontHeight+reflectionHeight(p)+shadowHeight(p),ret;if(p!=0){if(o.hAlign=='center'){if(p>0&&p<=Math.ceil((n-1)/2)){var front=setImageSize(p-1);w=o.backZoom*front.width;h=(o.backZoom*front.height)+reflectionHeight(p)+shadowHeight(p);}
...
$(document).ready(function(){
    jQuery(window).resize(function(){
        OnRunSlider(jQuery('#wiki-content-main').width(), 330);
    });    
});

function OnRunSlider(width, height) {
    $('.carousel').carousel({
        carouselWidth: width,
