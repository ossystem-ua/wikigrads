...
/**
 * sfGDAdapter provides a mechanism for creating thumbnail images. (extended to create square thumbnails by MINH)
...
class sfGDAdapter
{
  ...
  { 
    ...
    $this->square = array_key_exists('square', $this->options) ? $this->options['square'] : false; 
  ...
  public function loadFile($thumbnail, $image)
  {
    ...
    if (in_array($imgData['mime'], $this->imgTypes))
    {
      ...
      elseif ($this->square) 
        ...
        $this->minSize = min($this->sourceWidth, $this->sourceHeight);
        $src_x = ceil(($this->sourceWidth - $this->minSize) / 2);         
        //$src_y = ceil(($this->sourceHeight - $this->minSize) / 2);
        $src_y = 0;
                                
 	      imagecopyresampled($this->thumb, $this->source, 0, 0, $src_x, $src_y, $thumbnail->getThumbWidth(), $thumbnail->getThumbHeight(), $this->minSize, $this->minSize); 
      } 
      else
      {
