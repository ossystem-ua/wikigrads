 ...
 * @see    image_path
 */
function image_tag($source, $options = array(), $defValue = "")
{
  if (!$source)
  {
    return '';
  ...
  if (isset($options['src'])) {
    $options['src'] = setDefaultFile ($options['src'], $defValue);
  }
...
function checkImg ($url) {
    return true;
        $server = "http://www.".$_SERVER["SERVER_NAME"];        
    ...
    function setDefaultFile ($url, $defaultValue) {
        if (!checkImg ($url) && strlen($defaultValue) > 0) return $defaultValue;
        return $url;
    }