...
<div id="filter-block">
    <h2><?php echo $courseName; ?><span style="margin-left: 7px;"> | Live Feed</span></h2>
        ...
        <div class="wg-post-row-1">
            <table class="wg-post-name"><tr>
                <td class="wiki-left <?php if ($is_Staff == 1) { echo "wg-instructor"; } else { echo "wg-student"; }?>">
                    <?php echo link_to($user_name, '@user?id='.$profile->getId().'&name='.Utils::slugify($user_name), array('class' => 'tooltip3', 'title' => $user_name))?>
                </td>
    ...
    <div class="wg-post-block wg-block-pin-post">
        ...
        <div class="wg-post-col-2">
            ...
            <div class="wg-post-row-3">
                ...
                <a href="/ajax/add-comment/notification_id/<?php echo $n_id; ?>" data-id="<?php echo $n_id; ?>" class="href-cursor tooltip3 wg-none-href" title="Show comments" OnClick="ShowBlock(this, 'wiki-comment-list-', <?php echo $n_id; ?>, '<?php echo $name_show; ?>', '<?php echo $name_hide; ?>')">
                    <?php echo $name_show; ?>
                </a>
                ...
                <?php
                    "wg-flag-button new-flag tooltip4";
                    if ($notification['object']['flagget'] == 1) { $class = "wg-flag-button tooltip4 new-flag flagged"; }
                    ...
                    <div class="wg-post-col-2">
                        ...
                        <div class="wg-post-row-3">
                            ...
                            <?php
                                ...
                                if ($comment['flagget'] == 1) { $class = "wg-flag-button tooltip4 new-flag flagged"; }
...
<?php foreach($notifications as $n_id => $notification): ?>
    <?php // $postUserId = null; ?>
    <?php // if (isset($notification['object']['user_id'])): ?>
        <?php // $postUserId = Doctrine::getTable('sfGuardUser')->findOneBy("id", $notification['object']['user_id']); ?>
    <?php // endif; ?>
    <?php // if ($postUserId): ?>
        ...
        <?php
            ...
            switch($notification['object_name']) {
                case 'Post': 
                    if(!$IsPinned):
//                        include_partial('notification/list_item_post', array(
...
//                            'user_name' => $user_name));
                ...
                <?php                        
                    ...
                    break;
                case 'Document':
//                    include_partial('notification/list_item_document', array(
//                        'notification_id'=>$n_id, 
//                        'notification'=>$notification, 
//                        'timezone' => $timezone));
                    $isStaff = 0;
                ...
                <div class="wg-post-block">
                    <div class="wg-post-col-1 <?php if ($isStaff==1) { echo "wg-instructor"; } else { echo "wg-student"; }?>">
                        <?php echo link_to(image_tag(getUserThumbnail($doc_user->getId(), $notification['object']['user_image'], 50, 0, 0, 1)),'@user?id='.$notification['object']['user_id'].'&name='.Utils::slugify($user_name), array('class' => 'user-logo tooltip5', 'title' => $user_name))?>
                    </div>
                    <div class="wg-post-col-2">
                        <div class="wg-post-row-1">
                            <table class="wg-post-name"><tr>
